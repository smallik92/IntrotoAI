...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    pacmanPosition = gameState.getPacmanPosition()
    "*** YOUR CODE HERE ***"
    weightOfParticles = util.Counter()
    if noisyDistance is None:
        for i in range(self.numParticles):
            self.particles[i] = self.getJailPosition()
    else:
        for x in self.particles:
            weightOfParticles[x] += emissionModel[util.manhattanDistance(x,pacmanPosition)]
            
        weightOfParticles.normalize()
        #Resampling step
        if weightOfParticles.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(self.numParticles):
                self.particles[i] = util.sample(weightOfParticles)
  ...
  def elapseTime(self, gameState):
    ...
    for i in range(self.numParticles):
        ...
        self.particles[i] = util.sample(newPosDist[p])
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
        ...
        for j in range(self.numGhosts):
            curTuple = curTuple+(random.choice(self.legalPositions),)
        self.particles.append(curTuple)
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    weightOfParticles = util.Counter()
    #updates all particles so that the ghost appears in the prison cell
    for i in range(self.numGhosts):
    ...
    for p in self.particles:
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] is not None:
                weight *= emissionModels[i][util.manhattanDistance(p[i],pacmanPosition)]
        weightOfParticles[p] += weight
     #   weightOfParticles.normalize()
    ...
    if weightOfParticles.totalCount() == 0:
        ...
        for j in range(self.numParticles):
            self.particles[j] = tuple(util.sample(weightOfParticles))
