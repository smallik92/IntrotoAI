...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    distMostProbableGhost = {}
    for action in legal:
        nextPacmanPosition = Actions.getSuccessor(pacmanPosition,action)
        for ghostDist in livingGhostPositionDistributions:
            mostProbableGhostPosition = max(ghostDist, key = ghostDist.get)
            #for each ghost store the distance between the successor pacman position and the most probable next ghost position
            distMostProbableGhost[action] = self.distancer.getDistance(mostProbableGhostPosition,nextPacmanPosition)
            
            
    return min(distMostProbableGhost, key = distMostProbableGhost.get )
